...
//		printf("Integrity check dim(%d, %d, %d) start(%d, %d, %d) end(%d, %d, %d)\n", dimarray_3d[0], dimarray_3d[1], dimarray_3d[2], arr_start[0], arr_start[1], arr_start[2], arr_end[0], arr_end[1], arr_end[2]);
		...
		gettimeofday(&start, NULL);
		ret = meta_unpack_2d_face( NULL, NULL, dev_face[face_id], dev_data3, face_spec, g_type, async);
		gettimeofday(&end, NULL);
		...
		trans_dim[0] = (face_spec->size[2] == 1 ? face_spec->size[1] : face_spec->size[2]);
		trans_dim[1] = (face_spec->size[0] == 1 ? face_spec->size[1] : face_spec->size[0]);  
		trans_dim[2] = 1;
		...
		//TODO figure out what's wrong with transpose and re-enable
		gettimeofday(&start, NULL);
		ret = meta_transpose_2d_face(NULL, NULL, dev_face[(face_id & 1) ? face_id-1 : face_id+1], dev_face[face_id], &rtrans_dim, &rtrans_dim, g_type, async);
		gettimeofday(&end, NULL);
		printf("transpose time: %f\n", (end.tv_sec-start.tv_sec)*1000000.0+(end.tv_usec-start.tv_usec));
		//check_buffer(face[face_id], dev_face[face_id], face_spec->size[0]*face_spec->size[1]*face_spec->size[2]);
