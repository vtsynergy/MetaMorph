...
struct backend_handles {
  ...
  /** Handle from dlopen for whatever the OpenMP implementation library is, currently unused */
...
/**
 * Struct to hold CUDA wrapper function pointers that the core library needs to reference
 */
struct cuda_dyn_ptrs {
  ...
  /** Dynamically-loaded pointer to the function to initialize the n-th CUDA device*/
  a_err (* metaCUDAInitByID)(a_int);
  /** Dynamically-loaded pointer to the function to query the current CUDA device ID*/
  a_err (* metaCUDACurrDev)(a_int*);
  /** Dynamically-loaded pointer to the function to the function to validate the CUDA work sizes */
  a_err (* metaCUDAMaxWorkSizes)(a_dim3*, a_dim3*);
  /** Dynamically-loaded pointer to the function to flush all outstanding CUDA work */
  a_err (* metaCUDAFlush)();
  /** Dynamically-loaded pointer to the function to allocate and initialize two cudaEvent_ts for a meta_event */
  a_err (* metaCUDACreateEvent)(void**);
  /** Dynamically-loaded pointer to the function to destroy two cudaEvent_ts from a meta_event */
  a_err (* metaCUDADestroyEvent)(void*);
  /** Dynamically-loaded pointer to the function to register a callback through the CUDA machinery*/
...
/**
 * Struct to hold OpenCL wrapper function pointers that the core library needs to reference
 */
struct opencl_dyn_ptrs {
  ...
  /** Dynamically-loaded pointer to the function to get the current OpenCL device ID */
  ...
  /** Dynamically-loaded pointer to the function to allocate and initialize a cl_event for inclusion in a meta_event */
  a_err (* metaOpenCLCreateEvent)(void**);
  /** Dynamically-loaded pointer to the function to register a meta_callback through the OpenCL callback machinery */
...
/**
 * Struct to hold OpenMP wrapper function pointers that the core library needs to reference
 */
struct openmp_dyn_ptrs {
  ...
  /** Dynamically-loaded pointer to the function to finish any outstanding OpenMP work */
  ...
  /** Dynamically-loaded pointer to the function to register a callback function with the OpenMP backend */
...
a_err (* openmp_unpack_face)(size_t (*)[3], size_t (*)[3], void *, void *, meta_face *, int *, meta_type_id, int, meta_callback *, meta_event *, meta_event *, meta_event *, meta_event *);
  /** Dynamically-loaded pointer to the OpenMP 3D &-point Jacobi stencil function */
