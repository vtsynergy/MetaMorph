 ...
 * Intended for internal use to reuse staging buffers and internal data structures
 * \param size The size of the buffer needed
 * \param isHost Whether the buffer is a large host staging buffer (true) or a small internal data structure (small)
 */
...
//\todo FIXME: Do we need to take copies of compound operations' grid, block, and face, so the user can
...
/**
 * Enum which tells the queue node consumer what type of record to treat the union as
 */
typedef enum {
...
/**
 * The data elements needed for the send_packed callback for the initial D2H copy
 */
...
/**
 * The data elements needed for the pack_and_send callback for the initial pack kernel
 */
...
/**
 * The data elements needed for the recv_and_unpack MPI callback to transfer to the device then unpack
 */
...
/**
 * The data elements needed for the recv_packed MPI callback to transfer to the device
 */
 ...
 * \param argc The number of command line args provided to main (will be modified by MPI_Init)
 * \param argv The vector of command line character strings provided to main (will be modified by MPI_Init)
 */
...
/**
 * The Dequeue function, check if the oldest MPI request has finished, and if so, move it along
 * Does not permit out-of-order operations, will not do anything until the oldest request is done
 */
...
/**
 * Provide a wrapper to query the rank so that we both don't have to care which MPI library the backend is built against for dlopen
 * We also don't have to know/care what communicator the MPI plugin is using (if it changes in the future)
 ...
 * \param size A pointer to be filled with the sizeof the MPI data type, or NULL if not needed
 ...
 * \param packed_buf The MetaMorph-allocated device buffer to transfer (must be on current MetaMorph device)
 * \param buf_leng The length of the buffer to transfer (number of elements, not size)
 * \param tag A tag to apply to the transfer
 * \param req An address to return a request corresponding to the underlying MPI_Send/MPI_Isend
 * \param type The MetaMorph type the buffer contains
 * \param async Whether the transfer should be performed asynchronously (both the device-to-host, and process-to-process), or not
 */
void meta_mpi_packed_face_send(int dst_rank, void * packed_buf,
 ...
 * \param packed_buf The MetaMorph-allocated device buffer to receive to (must be on current MetaMorph device)
 * \param buf_leng The length of the buffer to receive (number of elements, not size)
 * \param tag A tag to match to the receive
 * \param req An address to return a request corresponding to the underlying MPI_Recv/MPI_Irecv
 * \param type The MetaMorph type the buffer contains
 * \param async Whether the transfer should be performed asynchronously (and process-to-process, and host-to-device), or not
 */
void meta_mpi_packed_face_recv(int src_rank, void * packed_buf,
		size_t buf_leng, int tag, MPI_Request * req, meta_type_id type,
		int async);

/**
 * Pack a face slab of a 3D MetaMorph device buffer, then send it to another process
 * Typically this is implemented via transparent host staging
 * \param grid_size The number of blocks to use for the pack (pack kernels are 1D)
 * \param block_size The muber of threads to use within each block of the pack (pack kernels are 1D) 
 * \param dst_rank The receiver's rank in MPI_COMM_WORLD
 * \param face The face specification of the 3D region to send
 * \param buf The MetaMorph-allocated 3D device buffer to pack from (must be on current MetaMorph device)
 * \param packed_buf A MetaMorph-allocated device buffer of sufficient size to fit the whole packed face for  transfer (must be on current MetaMorph device)
 * \param tag A tag to apply to the transfer
 * \param req An address to return a request corresponding to the underlying MPI_Send/MPI_Isend
 * \param type The MetaMorph type the buffer contains
 * \param async Whether the transfer should be performed asynchronously (The pack kernel, device-to-host transfer, and process-to-process transfer), or not
 */
 ...
 * Typically this is implemented via transparent host staging
 * \param grid_size The number of blocks to use for the unpack (unpack kernels are 1D)
 * \param block_size The muber of threads to use within each block of the unpack (unpack kernels are 1D) 
 * \param src_rank The sender's rank in MPI_COMM_WORLD
 * \param face The face specification of the 3D region to receive
 * \param buf The MetaMorph-allocated 3D device buffer to unpack to (must be on current MetaMorph device)
 * \param packed_buf The MetaMorph-allocated device buffer to receive to (must be on current MetaMorph device)
 * \param tag A tag to match to the receive
 * \param req An address to return a request corresponding to the underlying MPI_Recv/MPI_Irecv
 * \param type The MetaMorph type the buffer contains
