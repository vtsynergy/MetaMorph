...
/**
 * Store a single timing object (typically a single kernel run or a single transfer)
 */
typedef struct meta_timer {
  /** Record the name of the timing object, if relevant */
  char const * name;
  /** Timers are event-based, which abstract the various backend' event mechanisms, timer is associated with one */
  meta_event event;
  /** What global mode MetaMorph was running in when the timer was created */
  meta_preferred_mode mode;
  /** The size of whatever object is being timed (size of transfer, volume of the computed region, etc. */
  size_t size;
//\todo add level 3 items
} meta_timer;

/** All timing objects are maintained on separate queues, inside one of these nodes */
...
/** Each queue is named for reporting results, and needs to maintain its head and tail pointers */
...
/**
 * Create a timer, initializing an event on the underlying backend currently in use
 * \param ret_timer Address in which to return the allocated and initialized timer
 * \param mode The mode to initialize the timer in, typically whatever the current global mode is
 * \param size The size of the event being timed (transfer bytes, kernel volume, etc.)
 ...
 * \return hardcoded to zero
 * \todo Implement proper error codes
 * \bug FIXME: Doesn't currently destroy anything, it's just a wrapper around flush that then sets the initialization state to false
 */ 
