...
public:
  __device__
      ...
      float *
      getPointer() {
    extern __shared__ float s_fl[];
    return s_fl;
  }
};

/** specialization for unsigned long long */
template <> class SharedMem<unsigned long long> {
public:
  __device__
      /** Just get the address of the shared memory region
       * \return The start address of the shared memory region
      ...
      unsigned long long *
      getPointer() {
    extern __shared__ unsigned long long s_ul[];
    return s_ul;
  }
};
...
template <> class SharedMem<unsigned int> {
public:
  __device__
      /** Just get the address of the shared memory region
       * \return The start address of the shared memory region
      ...
      unsigned int *
      getPointer() {
    extern __shared__ unsigned int s_ui[];
    return s_ui;
  }
};
 ...
 */
template <>
__device__ double atomicAdd_wrapper<double>(double *addr, double val) {
  return atomicAdd(addr, val);
}
 ...
 */
template <typename T>
__global__ void kernel_dotProd(T *phi1, T *phi2, int i, int j, int k, int sx,
                               int sy, int sz, int ex, int ey, int ez, int gz,
                               T *reduction, int len_) {
  SharedMem<T> shared;
  ...
  for (itr = 0; itr < loads; itr++) {
    ...
    boundz = ((z >= sz) && (z <= ez));
    // if (boundx && boundy && boundz) psum[tid] += phi1[x+y*i+z*i*j] *
    // phi2[x+y*i+z*i*j];
  ...
  __syncthreads();
  // After accumulating the Z-dimension, have each block internally reduce X and
  // Y
  ...
  if (tid == 0)
    atomicAdd_wrapper<T>(reduction, psum[0]);
}
 ...
 */
template <typename T>
__global__ void kernel_reduction3(T *phi, int i, int j, int k, int sx, int sy,
...
/** Size of all children (>= level+1) so at level 0, child_size =
 * total_num_face_elements */
 ...
 * \param count the number of layers in the face structure (usually the number
 * of dimensions of the buffer)
 ...
 * \param count the number of layers in the face struct stored in constant
 * memory
 */
template <typename T>
__global__ void kernel_pack(T *packed_buf, T *buf, int size, int start,
                            int count) {
 ...
 */
template <typename T>
__global__ void kernel_unpack(T *packed_buf, T *buf, int size, int start,
                              int count) {
  int idx = threadIdx.x + blockDim.x * blockIdx.x;
  int nthreads = gridDim.x * blockDim.x;
  extern __shared__ int a[];
 ...
 * \param odata the output transposed 2D array (of size tran_width *
 * tran_height)
 ...
 */
template <typename T>
__global__ void kernel_transpose_2d(T *odata, T *idata, int arr_width,
  ...
  for (; bid < logicalBlocks; bid += gridBlocks) {
    ...
    // The blockDim.x+1 in R/W steps is to avoid bank conflicts if
    // blockDim.x==16 or 32
    ...
    // Added when the loop was added to ensure writes are finished before new
    // vals go into shared memory
    __syncthreads();
  }
}
 ...
 * 7) Optimizations: Read-only cache + Register blocking (Z) + smem blocking
 * (X-Y)
 ...
 * \warning works only with 2D thread blocks (use rectangular blocks, i.e. 64*4,
 * 128*2)
 */
template <typename T>
__global__ void kernel_stencil_3d7p(const T *__restrict__ ind,
                                    T *__restrict__ outd, int i, int j, int k,
                                    int sx, int sy, int sz, int ex, int ey,
                                    int ez, int gz, int len_) {
  SharedMem<T> shared;
...
a_err metaCUDAWrite(void *dst, void *src, size_t size, a_bool async,
  ...
  if (profiling_symbols.metaProfilingCreateTimer != NULL) {
    (*(profiling_symbols.metaProfilingCreateTimer))(&timer, metaModePreferCUDA,
                                                    size);
    if (events == NULL) {
      events = ((cudaEvent_t *)timer->event.event_pl);
    } else {
      // FIXME: are we leaking a created cudaEvent_t here since the profiling
      // function calls create? metaCUDADestroyEvent(frame->event.event_pl);
  ...
  // If a callback is provided, register it immediately after returning from
  // enqueuing the kernel
  if ((void *)call != NULL)
    cudaStreamAddCallback(0, metaCUDACallbackHelper, call, 0);
...
a_err metaCUDAInitByID(a_int accel) {
  ...
  // known to register destructor functions, so here we are re-registering our
  // global destructor /shrug
...
/**
 * This struct just allows the status and event values returned by the CUDA
...
void CUDART_CB metaCUDACallbackHelper(cudaStream_t stream, cudaError_t status,
                                      void *data) {
  if (data == NULL)
    return;
  meta_callback *payload = (meta_callback *)data;
  struct cuda_callback_data *info =
      (struct cuda_callback_data *)calloc(1, sizeof(struct cuda_callback_data));
  info->status = status;
  info->stream = stream;
...
a_err cuda_dotProd(size_t (*grid_size)[3], size_t (*block_size)[3], void *data1,
  ...
  case a_db:
    kernel_dotProd<double><<<grid, block, smem_len * sizeof(double)>>>(
        ...
        (*arr_end)[0], (*arr_end)[1], (*arr_end)[2], iters,
  ...
  case a_in:
    kernel_dotProd<int><<<grid, block, smem_len * sizeof(int)>>>(
        ...
        (*array_size)[2], (*arr_start)[0], (*arr_start)[1], (*arr_start)[2],
        (*arr_end)[0], (*arr_end)[1], (*arr_end)[2], iters, (int *)reduced_val,
        smem_len);
    break;
  ...
  case a_ui:
    kernel_dotProd<unsigned int>
        <<<grid, block, smem_len * sizeof(unsigned int)>>>(
            ...
            (*array_size)[1], (*array_size)[2], (*arr_start)[0],
            (*arr_start)[1], (*arr_start)[2], (*arr_end)[0], (*arr_end)[1],
            (*arr_end)[2], iters, (unsigned int *)reduced_val, smem_len);
    break;
  ...
  case a_ui:
    kernel_reduction3<unsigned int>
        <<<grid, block, smem_len * sizeof(unsigned int)>>>(
            ...
            (*array_size)[2], (*arr_start)[0], (*arr_start)[1], (*arr_start)[2],
            (*arr_end)[0], (*arr_end)[1], (*arr_end)[2], iters,
            (unsigned int *)reduced_val, smem_len);
    break;
...
a_err cuda_transpose_face(size_t (*grid_size)[3], size_t (*block_size)[3],
  ...
  default:
    fprintf(stderr, "Error: function 'cuda_transpose_face' not implemented for "
                    "selected type!\n");
    break;
  }
  if (events != NULL) {
...
a_err cuda_pack_face(size_t (*grid_size)[3], size_t (*block_size)[3],
  ...
  if (ret_event_c1 != NULL && ret_event_c1->mode == metaModePreferCUDA &&
  ...
  if (profiling_symbols.metaProfilingCreateTimer != NULL) {
    (*(profiling_symbols.metaProfilingCreateTimer))(
        &timer_c1, metaModePreferCUDA, get_atype_size(type) * 3);
    if (events_c1 == NULL) {
  ...
  if (ret_event_c2 != NULL && ret_event_c2->mode == metaModePreferCUDA &&
  ...
  if (profiling_symbols.metaProfilingCreateTimer != NULL) {
    (*(profiling_symbols.metaProfilingCreateTimer))(
        &timer_c2, metaModePreferCUDA, get_atype_size(type) * 3);
    if (events_c2 == NULL) {
  ...
  if (ret_event_c3 != NULL && ret_event_c3->mode == metaModePreferCUDA &&
  ...
  if (profiling_symbols.metaProfilingCreateTimer != NULL) {
    (*(profiling_symbols.metaProfilingCreateTimer))(
        &timer_c3, metaModePreferCUDA, get_atype_size(type) * 3);
    if (events_c3 == NULL) {
  ...
  if (grid_size == NULL || block_size == NULL) {
    block = dim3(256, 1, 1);
    grid = dim3((face->size[0] * face->size[1] * face->size[2] + block.x - 1) /
                    block.x,
                1, 1);
  } else {
  ...
  if (ret_event_k1 != NULL && ret_event_k1->mode == metaModePreferCUDA &&
  ...
  default:
    fprintf(stderr, "Error: function 'cuda_pack_face' not implemented for "
                    "selected type!\n");
    break;
  }
  if (events_k1 != NULL) {
...
a_err cuda_unpack_face(size_t (*grid_size)[3], size_t (*block_size)[3],
  ...
  // TODO: Create a grid/block similar to Kaixi's look at mpi_wrap.c to figure
  // out how size is computed
  ...
  default:
    fprintf(stderr, "Error: function 'cuda_unpack_face' not implemented for "
                    "selected type!\n");
    break;
  }
  if (events_k1 != NULL) {
  ...
  case a_ui:
    kernel_stencil_3d7p<unsigned int>
        <<<grid, block, smem_len * sizeof(unsigned int)>>>(
            ...
            (*array_size)[1], (*array_size)[2], (*arr_start)[0],
            (*arr_start)[1], (*arr_start)[2], (*arr_end)[0], (*arr_end)[1],
            (*arr_end)[2], iters, smem_len);
    break;

  default:
    fprintf(stderr, "Error: Function 'cuda_stencil_3d7p' not implemented for "
                    "selected type!\n");
    break;
  }
  if (events != NULL) {
