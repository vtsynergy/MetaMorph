...
#endif
/**
 * A simple wrapper around cudaMalloc to create a buffer on the current CUDA
 * device and pass it back up to the backend-agnostic layer
 ...
 * currently-running CUDA context
 ...
 * \param ret_event The address of a meta_event with initialized cudaEvent_t[2]
 ...
 * \param src The source buffer, a CUDA buffer allocated in MetaMorph's
 * currently-running CUDA context
 ...
 * \param block The requested number of work items within each workgroup in each
 * dimension
 ...
 * \param event The meta_event (bearing a dynamically-allocated cudaEvent_t[2]
 ...
 * a pass-though to send the callback back up to the backend-agnostic layer
...
void CUDART_CB metaCUDACallbackHelper(cudaStream_t stream, cudaError_t status,
                                      void *data);
/**
 * Since metaCUDACallbackHelper doesn't directly face the user, provide a way
 * for them to unpack the information it would receive from the CUDA for use in
 * their application
 * \param call The (already-triggered) callback to get the status of
 * \param ret_stream The address in which to copy the callback's triggering
 * stream
 ...
 * \param data1 first input array (a CUDA buffer residing on the
 * currently-active device)
 * \param data2 second input array (a CUDA buffer residing on the
 * currently-active device)
 ...
 * \param ret_event Address of an initialized event (with cudaEvent_t[2]
 ...
 * \param arr_end X, Y, and Z dimension end indicies for computing on a
 * subregion, to allow for a halo region
 * \param reduced_val the final dotproduct output (scalar) across all blocks,
 ...
 * \param call Register a callback to be automatically invoked when the kernel
 * finishes, or NULL if none
 * \param ret_event_k1 Address of an initialized event (with cudaEvent_t[2]
 * payload) in which to return the kernel event used for asynchronous calls and
 * timing
 * \param ret_event_c1 Address of an initialized event (with cudaEvent_t[2]
 * payload) in which to return the event for the cudaMemcpyToSymbol of
 * face->size event used for asynchronous calls and timing
 * \param ret_event_c2 Address of an initialized event (with cudaEvent_t[2]
 * payload) in which to return the event for the cudaMemcpyToSymbol of
 * face->stride event used for asynchronous calls and timing
 * \param ret_event_c3 Address of an initialized event (with cudaEvent_t[2]
 * payload) in which to return the event for the cudaMemcpyToSymbol of
 * remain_dim to c_face_child_size event used for asynchronous calls and timing
