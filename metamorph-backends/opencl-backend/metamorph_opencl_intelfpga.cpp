...
// If we need to though, it would likely be sufficient to CAS the num_platforms
// int or platforms pointer
...
void metaOpenCLQueryDevices() {
  ...
  for (i = 0; i < num_platforms; i++) {
    temp_uint = 0;
    fprintf(stderr,
            "OCL DEBUG: clGetDeviceIDs Count query on platform[%d] has "
            "address[%x]!\n",
            i, &temp_uint);
    if (clGetDeviceIDs(platforms[i], CL_DEVICE_TYPE_ALL, 0, NULL, &temp_uint) !=
        CL_SUCCESS)
  ...
  for (i = 0; i < num_platforms; i++) {
    fprintf(stderr,
            "OCL DEBUG: clGetDeviceIDs IDs query on platform[%d] has "
            "addresses[%x][%x]!\n",
  ...
  // something... free(devices); free(platforms);
}
...
cl_int metaOpenCLBuildProgram(metaOpenCLStackFrame *frame) {
  ...
  frame->program_opencl_core = clCreateProgramWithBinary(
      frame->context, 1, &(frame->device), &metaCLProgLen,
      (const unsigned char **)&metaCLProgSrc, NULL, NULL);
#endif
  ...
  if (getenv("METAMORPH_MODE") != NULL) {
    if (strcmp(getenv("METAMORPH_MODE"), "OpenCL") == 0) {
      size_t needed = snprintf(NULL, 0,
                               "-I . -D TRANSPOSE_TILE_DIM=(%d) -D OPENCL -D "
                               "TRANSPOSE_TILE_BLOCK_ROWS=(%d)",
      ...
      size_t needed =
          snprintf(NULL, 0,
                   "-I . -D TRANSPOSE_TILE_DIM=(%d) -D OPENCL -D "
                   "TRANSPOSE_TILE_BLOCK_ROWS=(%d) -g -cl-opt-disable",
                   TRANSPOSE_TILE_DIM, TRANSPOSE_TILE_BLOCK_ROWS);
      args = (char *)malloc(needed);
      snprintf(args, needed,
               "-I . -D TRANSPOSE_TILE_DIM=(%d) -D OPENCL -D "
  ...
  free(log);
  // Not creating all the Kernels. Just the Kernels supposed to be optimized for
  // FPGA. Reduction Dot product Transpose Pack Unpack Stencil csr crc
  ...
  frame->kernel_reduce_db =
      clCreateKernel(frame->program_opencl_core, "kernel_reduce_db", NULL);
#endif // KERNEL_REDUCE
#ifdef KERNEL_DOT_PROD
  frame->kernel_dotProd_db =
      clCreateKernel(frame->program_opencl_core, "kernel_dotProd_db", NULL);
...
#ifdef KERNEL_PACK
  frame->kernel_pack_2d_face_db =
      clCreateKernel(frame->program_opencl_core, "kernel_pack_db", NULL);
#endif // KERNEL_PACK
#ifdef KERNEL_UPACK
  frame->kernel_unpack_2d_face_db =
      clCreateKernel(frame->program_opencl_core, "kernel_unpack_db", NULL);
#endif // KERNEL_UPACK
#ifdef KERNEL_STENCIL
  frame->kernel_stencil_3d7p_db = clCreateKernel(
      frame->program_opencl_core, "kernel_stencil_3d7p_db", NULL);
#endif // KERNL_STENCIL
#ifdef KERNEL_CSR
  frame->kernel_csr_fl =
      clCreateKernel(frame->program_opencl_core, "kernel_csr_fl", &err);
  CHKERR(err, "Failed to create a compute kernel!");
#endif // KERNL_CSR
#ifdef KERNEL_CRC
  frame->kernel_crc_ui =
      clCreateKernel(frame->program_opencl_core, "kernel_crc_ui", &err);
  CHKERR(err, "Failed to create a compute kernel!");
#endif // KERNL_CRC
#endif // FPGA_DOUBLE
  ...
  frame->kernel_dotProd_fl =
      clCreateKernel(frame->program_opencl_core, "kernel_dotProd_fl", NULL);
#endif // KERNEL_REDUCE
#ifdef KERNEL_DOT_PROD
  frame->kernel_reduce_fl =
      clCreateKernel(frame->program_opencl_core, "kernel_reduce_fl", NULL);
#endif // KERNEL_DOT_PROD
#ifdef KERNEL_TRANSPOSE
...
#endif // KERNEL_TRANSPOSE
#ifdef KERNEL_PACK
  frame->kernel_pack_2d_face_fl =
      clCreateKernel(frame->program_opencl_core, "kernel_pack_fl", NULL);
#endif // KERNEL_PACK
#ifdef KERNEL_UPACK
  frame->kernel_unpack_2d_face_fl =
      clCreateKernel(frame->program_opencl_core, "kernel_unpack_fl", NULL);
#endif // KERNEL_UPACK
#ifdef KERNEL_STENCIL
  frame->kernel_stencil_3d7p_fl = clCreateKernel(
      frame->program_opencl_core, "kernel_stencil_3d7p_fl", NULL);
#endif // KERNL_STENCIL
#ifdef KERNEL_CSR
  frame->kernel_csr_fl =
      clCreateKernel(frame->program_opencl_core, "kernel_csr_fl", &err);
  ...
  frame->kernel_reduce_ul =
      clCreateKernel(frame->program_opencl_core, "kernel_reduce_ul", NULL);
#endif // KERNEL_REDUCE
#ifdef KERNEL_DOT_PROD
  frame->kernel_dotProd_ul =
      clCreateKernel(frame->program_opencl_core, "kernel_dotProd_ul", NULL);
#endif // KERNEL_DOT_PROD
#ifdef KERNEL_TRANSPOSE
...
#endif // KERNEL_TRANSPOSE
#ifdef KERNEL_PACK
  frame->kernel_pack_2d_face_ul =
      clCreateKernel(frame->program_opencl_core, "kernel_pack_ul", NULL);
#endif // KERNEL_PACK
#ifdef KERNEL_UPACK
  frame->kernel_unpack_2d_face_ul =
      clCreateKernel(frame->program_opencl_core, "kernel_unpack_ul", NULL);
#endif // KERNEL_UPACK
#ifdef KERNEL_STENCIL
  frame->kernel_stencil_3d7p_ul = clCreateKernel(
      frame->program_opencl_core, "kernel_stencil_3d7p_ul", NULL);
#endif // KERNL_STENCIL
#ifdef KERNEL_CSR
  frame->kernel_csr_fl =
      clCreateKernel(frame->program_opencl_core, "kernel_csr_fl", &err);
  ...
  frame->kernel_reduce_in =
      clCreateKernel(frame->program_opencl_core, "kernel_reduce_in", NULL);
#endif // KERNEL_REDUCE
#ifdef KERNEL_DOT_PROD
  frame->kernel_dotProd_in =
      clCreateKernel(frame->program_opencl_core, "kernel_dotProd_in", NULL);
#endif // KERNEL_DOT_PROD
#ifdef KERNEL_TRANSPOSE
...
#endif // KERNEL_TRANSPOSE
#ifdef KERNEL_PACK
  frame->kernel_pack_2d_face_in =
      clCreateKernel(frame->program_opencl_core, "kernel_pack_in", NULL);
#endif // KERNEL_PACK
#ifdef KERNEL_UPACK
  frame->kernel_unpack_2d_face_in =
      clCreateKernel(frame->program_opencl_core, "kernel_unpack_in", NULL);
#endif // KERNEL_UPACK
#ifdef KERNEL_STENCIL
  frame->kernel_stencil_3d7p_in = clCreateKernel(
      frame->program_opencl_core, "kernel_stencil_3d7p_in", NULL);
#endif // KERNL_STENCIL
#ifdef KERNEL_CSR
  frame->kernel_csr_fl =
      clCreateKernel(frame->program_opencl_core, "kernel_csr_fl", &err);
  ...
  frame->kernel_reduce_ui =
      clCreateKernel(frame->program_opencl_core, "kernel_reduce_ui", NULL);
#endif // KERNEL_REDUCE
#ifdef KERNEL_DOT_PROD
  frame->kernel_dotProd_ui =
      clCreateKernel(frame->program_opencl_core, "kernel_dotProd_ui", NULL);
#endif // KERNEL_DOT_PROD
#ifdef KERNEL_TRANSPOSE
...
#endif // KERNEL_TRANSPOSE
#ifdef KERNEL_PACK
  frame->kernel_pack_2d_face_ui =
      clCreateKernel(frame->program_opencl_core, "kernel_pack_ui", NULL);
#endif // KERNEL_PACK
#ifdef KERNEL_UPACK
  frame->kernel_unpack_2d_face_ui =
      clCreateKernel(frame->program_opencl_core, "kernel_unpack_ui", NULL);
#endif // KERNEL_UPACK
#ifdef KERNEL_STENCIL
  frame->kernel_stencil_3d7p_ui = clCreateKernel(
      frame->program_opencl_core, "kernel_stencil_3d7p_ui", NULL);
#endif // KERNL_STENCIL
#ifdef KERNEL_CSR
  frame->kernel_csr_fl =
      clCreateKernel(frame->program_opencl_core, "kernel_csr_fl", &err);
...
// For this push to be both exposed and safe from hazards, it must make a copy
// of the frame
// such that the user never has access to the pointer to the frame that's
// actually on the shared stack object
...
void metaOpenCLPushStackFrame(metaOpenCLStackFrame *frame) {
  ...
  metaOpenCLStackNode *newNode =
      (metaOpenCLStackNode *)malloc(sizeof(metaOpenCLStackNode));
...
metaOpenCLStackFrame *metaOpenCLTopStackFrame() {
  metaOpenCLStackFrame *frame =
      (metaOpenCLStackFrame *)malloc(sizeof(metaOpenCLStackFrame));
...
cl_int metaOpenCLDestroyStackFrame(metaOpenCLStackFrame *frame) {
  ...
  // TODO since this only destroys a frame, we must release the global program
  // source elsewhere
}
...
cl_int opencl_dotProd(size_t (*grid_size)[3], size_t (*block_size)[3],
  ...
  default:
    ...
    break;
       ...
       // (*arr_end)[1], (*arr_end)[0], (*arr_end)[2]); printf("SMEM: %d\n",
       // smem_len);
...
cl_int opencl_pack_face(size_t (*grid_size)[3], size_t (*block_size)[3],
                        ...
                        cl_event *event_k1, cl_event *event_c1,
                        cl_event *event_c2, cl_event *event_c3) {
  cl_int ret;
  cl_kernel kern;
...
// TODO Timing needs to be made consistent with CUDA (ie the event should return
// time for copying to constant memory and the kernel
#ifdef KERNEL_PACK
  switch (type) {
#ifdef FPGA_DOUBLE
  case meta_db:
...
cl_int opencl_csr(size_t global_size, size_t local_size, void *csr_ap,
                  void *csr_aj, void *csr_ax, void *x_loc, void *y_loc,
                  meta_type_id type, int async, cl_event *wait,
                  cl_event *event) {
  ...
  err = clEnqueueNDRangeKernel(frame->queue, kern, 1, NULL, &grid, &block, 1,
                               wait, event);
  CHKERR(err, "Failed to compute kernel.....");
  // ret |= clEnqueueNDRangeKernel(frame->queue, kern, 1, NULL, &grid, &block,
  // 0, NULL, event);
...
cl_int opencl_crc(size_t global_size, size_t local_size, void *dev_input,
