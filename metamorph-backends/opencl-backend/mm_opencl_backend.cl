 ...
 * \param psum An array of at least _len initialized values to accumulate into
 * the 0th cell
...
#if (defined(UNSIGNED_LONG) &&                                                 \
     (defined(KERNEL_REDUCE) || defined(KERNEL_DOT_PROD)))
/**
...
#if (defined(DOUBLE) && (defined(KERNEL_REDUCE) || defined(KERNEL_DOT_PROD)))
/**
 * Manual implementation of a double-precision atomic add based on an atomic
 * compare and exchange
 * \param address The location of the initialized double precision value to
 * accumulate to
...
double atomicAdd_db(__global double *address, double val) {
  __global unsigned long *address_as_ul = (__global unsigned long *)address;
  // unsigned long old = *address_as_ul, assumed;
  volatile unsigned long old = atom_add(address_as_ul, 0), assumed;
  do {
    assumed = old;
    old =
        atom_cmpxchg(address_as_ul, assumed, as_long(val + as_double(assumed)));
  } while (assumed != old);
...
#if (defined FLOAT && (defined(KERNEL_REDUCE) || defined(KERNEL_DOT_PROD)))
/**
 * Manual implementation of a single-precision atomic add based on an atomic
 * compare and exchange
 * \param address The location of the initialized double precision value to
 * accumulate to
 * \param val The value to add to that already stored in address
...
/**
 * Double precision dot-product of identically-shaped subregions of two
 * identically-shaped 3D arrays this kernel works for 3D data only.
 ...
 * \param gz the number of iterations necessary to fully-compute the Z dimension
 * in case the global work size requires more than one element per workitem
 * \param reduction the final dotproduct output (scalar) across all workgroups,
 * assumed to be initialized before the kernel
 * \param len_ the length of the dynamically allocated location region, psum
 * \param psum a dynamically-allocated local memory region for computing thread
 * and work-group partial results
 */
__kernel void kernel_dotProd_db(__global double *phi1, __global double *phi2,
  ...
  if (tid == 0)
    atomicAdd_db(reduction, psum[0]);
}
#endif

#if (defined(FLOAT) && defined(KERNEL_DOT_PROD))
/**
 * Single precision dot-product of identically-shaped subregions of two
 * identically-shaped 3D arrays this kernel works for 3D data only.
 * \param phi1 first input array
 * \param phi2 second input array
 * \param i arrays size in the X dimension
...
/**
 * Unsigned long dot-product of identically-shaped subregions of two
 * identically-shaped 3D arrays this kernel works for 3D data only.
 * \param phi1 first input array
 * \param phi2 second input array
 * \param i arrays size in the X dimension
 ...
 * and work-group partial results
 */
__kernel void kernel_dotProd_ul(__global unsigned long *phi1,
                                __global unsigned long *phi2, int i, int j,
                                int k, int sx, int sy, int sz, int ex, int ey,
                                int ez, int gz,
                                __global unsigned long *reduction, int len_,
                                __local volatile unsigned long *psum) {
  ...
  if (tid == 0)
    atom_add(reduction, psum[0]);
}
#endif

#if (defined(INTEGER) && defined(KERNEL_DOT_PROD))
/**
 * Integer dot-product of identically-shaped subregions of two
 * identically-shaped 3D arrays this kernel works for 3D data only.
 * \param phi1 first input array
 * \param phi2 second input array
 * \param i arrays size in the X dimension
...
/**
 * Unsigned integer dot-product of identically-shaped subregions of two
 * identically-shaped 3D arrays this kernel works for 3D data only.
 * \param phi1 first input array
 * \param phi2 second input array
 * \param i arrays size in the X dimension
 ...
 * and work-group partial results
 */
__kernel void kernel_dotProd_ui(__global unsigned int *phi1,
                                __global unsigned int *phi2, int i, int j,
                                int k, int sx, int sy, int sz, int ex, int ey,
                                int ez, int gz,
                                __global unsigned int *reduction, int len_,
                                __local volatile unsigned int *psum) {
 ...
 * If a subregion is utilized, it is assumed to start at zero and extend to
 * tran_width-1 and tran_height-1 Any ghost elements outside the selected
 * subregion aren't touched idata and odata must not overlap, does not support
 * in-place transpose
 ...
 * \param tile a dynamically-allocated local memory region of size
 * (get_local_size(0)+1)*get_local_size(1)*get_local_size(2)
 */
__kernel void kernel_transpose_2d_face_db(__global double *odata,
 ...
 * (get_local_size(0)+1)*get_local_size(1)*get_local_size(2)
 */
__kernel void kernel_transpose_2d_face_fl(__global float *odata,
 ...
 * (get_local_size(0)+1)*get_local_size(1)*get_local_size(2)
 */
__kernel void kernel_transpose_2d_face_ul(__global unsigned long *odata,
 ...
 * (get_local_size(0)+1)*get_local_size(1)*get_local_size(2)
 */
__kernel void kernel_transpose_2d_face_in(__global int *odata,
                                          __global int *idata, int arr_width,
                                          int arr_height, int tran_width,
                                          int tran_height, __local int *tile) {
 ...
 * (get_local_size(0)+1)*get_local_size(1)*get_local_size(2)
 */
__kernel void kernel_transpose_2d_face_ui(__global unsigned int *odata,
...
/**
 * Internal function to translate a thread ID to the corresponding linear index
 * into a 3D array, based on a specified face, used by the pack and unpack
 * kernels.
 * \param tid The index in the packed linear buffer
 * \param a A dynamically-allocated local memory array used to calculate partial
 * offsets during the level iteration
 * \param start The initial index offset
 * \param count The number of elements in the size, stride, and child_size
 * arrays
 ...
 * \param c_face_child_size The number of samples in the next tree level and all
 * its descendants
...
/**
 * Kernel to pack a specified slab of a 3D buffer into a packed format for
 * communication over the network. Can be used with a unit-thick Slab to produce
 * a 2D array for transposition
 * \param packed_buf The output buffer, of sufficient size to store the entire
 * slab
 ...
 * \param c_face_child_size The number of samples in the next tree level and all
 * its descendants
 * \param a A dynamically-allocated local memory array used to during pack
 * offset calculation
 */
__kernel void kernel_pack_2d_face_db(__global double *packed_buf,
 ...
 * offset calculation
 */
__kernel void kernel_pack_2d_face_fl(__global float *packed_buf,
                                     __global float *buf, int size, int start,
                                     int count, __constant int *c_face_size,
                                     __constant int *c_face_stride,
                                     __constant int *c_face_child_size,
                                     __local int *a) {
 ...
 * offset calculation
 */
__kernel void kernel_pack_2d_face_ul(__global unsigned long *packed_buf,
                                     __global unsigned long *buf, int size,
                                     int start, int count,
 ...
 * offset calculation
 */
__kernel void kernel_pack_2d_face_in(__global int *packed_buf,
                                     __global int *buf, int size, int start,
                                     int count, __constant int *c_face_size,
                                     __constant int *c_face_stride,
                                     __constant int *c_face_child_size,
                                     __local int *a) {
 ...
 * offset calculation
 */
__kernel void kernel_pack_2d_face_ui(__global unsigned int *packed_buf,
                                     __global unsigned int *buf, int size,
                                     int start, int count,
                                     __constant int *c_face_size,
                                     __constant int *c_face_stride,
                                     __constant int *c_face_child_size,
...
/**
 * Kernel to unpack a specified slab of a 3D buffer from a packed format likely
 * communicated over the network. Can be used with a unit-thick slab to populate
 * a portion of a 3D region from a 2D array
 * \param packed_buf The inpu buffer, of sufficient size to store the entire
 * slab
 * \param buf The unpacked 3D buffer
 * \param size The length of packed_buf
 * \param start The initial index offset
 ...
 * offset calculation
 */
__kernel void kernel_unpack_2d_face_db(
    __global double *packed_buf, __global double *buf, int size, int start,
    int count, __constant int *c_face_size, __constant int *c_face_stride,
    __constant int *c_face_child_size, __local int *a) {
  int idx = get_global_id(0);
  const int nthreads = get_global_size(0);
  // this loop handles both nthreads > size and nthreads < size
  for (; idx < size; idx += nthreads)
 ...
 * offset calculation
 */
__kernel void kernel_unpack_2d_face_fl(__global float *packed_buf,
                                       __global float *buf, int size, int start,
                                       int count, __constant int *c_face_size,
                                       __constant int *c_face_stride,
                                       __constant int *c_face_child_size,
 ...
 * offset calculation
 */
__kernel void kernel_unpack_2d_face_ul(__global unsigned long *packed_buf,
                                       __global unsigned long *buf, int size,
                                       int start, int count,
                                       __constant int *c_face_size,
                                       __constant int *c_face_stride,
 ...
 * offset calculation
 */
__kernel void kernel_unpack_2d_face_in(__global int *packed_buf,
                                       __global int *buf, int size, int start,
                                       int count, __constant int *c_face_size,
                                       __constant int *c_face_stride,
                                       __constant int *c_face_child_size,
 ...
 * offset calculation
 */
__kernel void kernel_unpack_2d_face_ui(__global unsigned int *packed_buf,
                                       __global unsigned int *buf, int size,
                                       int start, int count,
                                       __constant int *c_face_size,
                                       __constant int *c_face_stride,
...
/**
 * Kernel to perform a single step of a 3D 7-point Jacobi stencil, averaging the
 * value of the 7 cells into the center this kernel works for 3D data only,
 * assumes a minimum of 1-element halo region exists on all sides
 ...
 * \param gz The number of iterations necessary to cover the entire Z dimesion
 * if the global work size can't do it in one
 * \param len_ the length of the bind array (only used by V2)
 * \param bind A dynamically allocated local memory array used for local memory
 * blocking (only used by V2)
 */
...
#pragma unroll 8
  for (; z < gz; z++) {
    ...
    if (boundx && boundy && boundz)
      outd[c] =
          (rz1 + ind[c - 1] + ind[c - i] + r0 + ind[c + i] + ind[c + 1] + rz2) /
          (double)7;
    c += ij;
    rz1 = r0;
    r0 = rz2;
...
__kernel void kernel_stencil_3d7p_db_v0(const __global double *__restrict__ ind,
                                        __global double *__restrict__ outd,
                                        int i, int j, int k, int sx, int sy,
                                        int sz, int ex, int ey, int ez, int gz,
                                        int len_, __local double *bind) {
  int x, y, z, itr;
  bool boundx, boundy, boundz;
...
#if (0 && defined(DOUBLE) && defined(KERNEL_STENCIL_V2))
__kernel void kernel_stencil_3d7p_db_v2(const __global double *__restrict__ ind,
                                        __global double *__restrict__ outd,
                                        int i, int j, int k, int sx, int sy,
                                        int sz, int ex, int ey, int ez, int gz,
                                        int len_, __local double *bind) {
...
#pragma unroll 8
  for (; z < gz; z++) {
    ...
    if (boundx && boundy && boundz)
      outd[c] = (rz1 + bind[bc - 1] + bind[bc - bi] + r0 + bind[bc + bi] +
                 bind[bc + 1] + rz2) /
                (double)7;
    c += ij;
    rz1 = r0;
...
__kernel void kernel_csr_db(const unsigned int num_rows,
                            ...
                            __global unsigned int *Aj, __global double *Ax,
...
__kernel void kernel_csr_fl(const unsigned int num_rows,
                            __global unsigned int *Ap,
                            __global unsigned int *Aj, __global float *Ax,
 ...
 */
__kernel void
kernel_csr_ul(const unsigned int num_rows, __global unsigned int *Ap,
              __global unsigned int *Aj, __global unsigned long *Ax,
              __global unsigned long *x, __global unsigned long *y) {
...
__kernel void kernel_csr_in(const unsigned int num_rows,
                            __global unsigned int *Ap,
                            __global unsigned int *Aj, __global int *Ax,
...
__kernel void kernel_csr_ui(const unsigned int num_rows,
                            ...
                            __global unsigned int *Ax, __global unsigned int *x,
...
#ifdef KERNEL_CRC
// TODO either include this in the repo or make it required and found before
// build
#define OPENCL
...
// CRC kernel
#if ((defined(DOUBLE) || defined(FLOAT) || defined(UNSIGNED_LONG) ||           \
      defined(INTEGER) || defined(UNSIGNED_INTEGER)) &&                        \
     defined(KERNEL_CRC))
/**
 * Kernel to compute a cyclic redundancy check on a stretch of binary data
 * Only supports 1D worksizes and has no concept of the main MetaMorph data
 * types Uses the canonical Slice-by-8 algorithm
 * \todo FIXME This looks like a single-work-item variant, need to validate and
 * ensure an NDRange version also exists
 * \param data The data buffer to evaluate
 * \param length_bytes Number of bytes for each thread to process
 * \param length_ints Number of integers for each thread to process (should be
 * length_bytes/sizeof(uint))
...
__kernel void kernel_crc_ui(__global const uint *restrict data,
                            uint length_bytes, const uint length_ints,
                            uint num_pages, __global uint *restrict res) {
