...
#define METAMORPH_OCL_DEFAULT_BLOCK                                            \
  { 16, 8, 1 }
#endif
...
#if (!defined(FPGA_DOUBLE) && !defined(FPGA_FLOAT) &&                          \
     !defined(FPGA_UNSIGNED_LONG) && !defined(FPGA_INTEGER) &&                 \
     !defined(FPGA_UNSIGNED_INTEGER))
#error Macro is Undefined,Please define one of FPGA_DOUBLE, FPGA_FLOAT, FPGA_UNSIGNED_LONG, FPGA_INTEGER, FPGA_UNSIGNED_INTEGER
     ...
     !defined(KERNEL_TRANSPOSE) && !defined(KERNEL_PACK) &&                    \
     !defined(KERNEL_UPACK) && !defined(KERNEL_STENCIL) &&                     \
     !defined(KERNEL_CSR) && !defined(KERNEL_CRC))
...
// respectively. All Stack operations (push, top and pop) make a copy of the
// frame's contents, therefore,
// the frame itself is thread-private, and even if tampered with, cannot affect
// the thread safety of the shared stack object.
// For basic CPU/GPU/MIC devices, the provided initialization calls should be
// sufficient and
// frame objects should not need to be directly modified; however, for FPGA
// devices which must use clBuildProgramFromBinary, the user will need to
// implement analogues to metaOpenCLInitStackFrame and
// metaOpenCLDestroyStackFrame, which appropriately create and release all
// necessary frame members. If implemented correctly, the built-in hazard-aware
...
// TODO these shouldn't need to be exposed to the user, unless there's a CUDA
// call we need to emulate
