...
/** The OpenMP event type, for now it's just a mapping to a time management struct */
 ...
 * \param grid_size The number of blocks in each global dimension (outer loop bounds, currently ignored)
 * \param block_size The dimensions of each block (inner loop bounds, currently ignored)
 * \param data1 first input array (an OpenMP buffer residing on the currently-active device)
 * \param data2 second input array (an OpenMP buffer residing on the currently-active device)
 ...
 * \param reduction_var the final dotproduct output (scalar) across all workgroups, assumed to be initialized before the kernel (an OpenMP buffer residing on the currently-active device)
 * \param type The supported MetaMorph data type that data1, data2, and reduced_val contain (Currently: a_db, a_fl, a_ul, a_in, a_ui)
 * \param async Whether the kernel should be run asynchronously or blocking (Currently NOOP, all OpenMP operations run synchronously)
 * \param call Register a callback to be automatically invoked when the kernel finishes, or NULL if none
 * \param ret_event Address of an initialized event (with openmpEvent[2] payload) in which to return the kernel's wrapping events
 * \return 0 on success
 ...
 * \param grid_size The number of blocks in each global dimension (outer loop bounds, currently ignored)
 * \param block_size The dimensions of each block (inner loop bounds, currently ignored)
 * \param data input array (an OpenMP buffer residing on the currently-active device)
 * \param array_size X, Y, and Z dimension sizes of the input arrays (must match)
 * \param arr_start X, Y, and Z dimension start indicies for computing on a subregion, to allow for a halo region
 * \param arr_end X, Y, and Z dimension end indicies for computing on a subregion, to allow for a halo region
 * \param reduction_var the final dotproduct output (scalar) across all workgroups, assumed to be initialized before the kernel (an OpenMP buffer residing on the currently-active device)
 ...
 * \param tran_dim_xy the X and Y dimensions of outdata, Z is ignored (currently ignored)
 ...
 * \param async Whether the kernel should be run asynchronously or blocking (Currently NOOP, all OpenMP operations run synchronously)
 * \param call Register a callback to be automatically invoked when the kernel finishes, or NULL if none
 * \param ret_event_k1 Address of an initialized event (with openmpEvent[2] payload) in which to return the kernel event used for asynchronous calls and timing
 * \param ret_event_c1 Address of an initialized event (with openmpEvent[2] payload) in which to return the event for copying face->size used for asynchronous calls and timing (currently unused in OpenMP)
 * \param ret_event_c2 Address of an initialized event (with openmpEvent[2] payload) in which to return the event for copying face->stride used for asynchronous calls and timing (currently unused in OpenMP)
 * \param ret_event_c3 Address of an initialized event (with openmpEvent[2] payload) in which to return the event for copying remain_dim to c_face_child_size event used for asynchronous calls and timing (currently unused in OpenMP)
 * \return 0 on success
 * \todo implement OpenMP error codes
