...
/** Maintain a global state variable for the currently-in-use backend/execution mode */
...
/** Implementation of the internal module-management list type for recording known modules */
 ...
 * \return true if the provided record is non-NULL and already in the list, false otherwise
...
/**
 * The MetaMorph destructor, its primary purpose right now is just to call the function that tears down dynamically-loaded backends and plugins in the correct order
 */
void meta_finalize() {
  meta_close_libs();
}
/**
 * The MetaMorph constructor, its primary purpose is to search for any installed backends and plugins and load them
 * All plugins should check if the core library is loaded by checking core_capability for a state other than module_uninitialized, and if not call this
 . This should ensure that no matter what order things are linked, they are initialized in the correct order
 * This function is kept separate in expectation that eventually there will be initialization functionality that is not related to library loading
 */
...
/**
 * Create the face structure for a given face and slab thickness of a contiguous row-major 3D region
 * 0 = XY face at the origin
 * 1 = XY face at the far side
 * 2 = YZ face at the origin
 * 3 = YZ face at the far side
 * 4 = XZ face at the origin
...
a_err meta_csr(a_dim3 * grid_size, a_dim3 * block_size, size_t global_size, void * csr_ap, void * csr_aj, void * csr_ax, void * x_loc, void * y_loc, meta_type_id type, a_bool async,
		meta_callback *call, meta_event * ret_event) {
	a_err ret;
