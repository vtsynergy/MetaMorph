...
void meta_load_libs() {
  ...
  backends.cuda_be_handle =
      dlopen("libmm_cuda_backend.so", RTLD_NOW | RTLD_GLOBAL);
  ...
  backends.opencl_be_handle =
      dlopen("libmm_opencl_backend.so", RTLD_NOW | RTLD_GLOBAL);
  ...
  backends.openmp_be_handle =
      dlopen("libmm_openmp_backend.so", RTLD_NOW | RTLD_GLOBAL);
  ...
  if (plugins.mpi_handle != NULL) {
    ...
    CHECKED_DLSYM("libmm_mpi.so", plugins.mpi_handle, "meta_mpi_finalize",
                  mpi_symbols.meta_mpi_finalize);
    CHECKED_DLSYM("libmm_mpi.so", plugins.mpi_handle, "finish_mpi_requests",
                  mpi_symbols.finish_mpi_requests);
  }
  plugins.profiling_handle =
      dlopen("libmm_profiling.so", RTLD_NOW | RTLD_GLOBAL);
  if (plugins.profiling_handle != NULL) {
    core_capability |= module_implements_profiling;
    CHECKED_DLSYM("libmm_profiling.so", plugins.profiling_handle,
