...
/**
 * Should never be user-edited, it's just for masking the bits needed for fast
 * indexing
 */
#define POOL_MASK (META_MPI_POOL_SIZE - 1)
/** Manage a reusable ring queue pool of host-staging buffers, since
 * applications frequently transfer the same setof buffers between stages */
void *host_buf_pool[META_MPI_POOL_SIZE];
/** The pool implmentation needs to know how big each allocation was to know if
 * the one in this slot will fit, or zero if it is unoccupied */
unsigned long host_buf_pool_size[META_MPI_POOL_SIZE];
/** A monotonically increasing token that is masked by the ring queue size to
 * represent the "front" of the queue */
unsigned long host_pool_token = 0;
#ifdef WITH_MPI_POOL_TIMING
/** An internal counter for time spent performing staging buffer pool
 * operations. (Currently unused) */
double host_time = 0.0;
#endif

/** Manage an internal pool for our frequently used request and callback
 * payloads just like host buffers */
...
a_err metaMPIRank(int *rank) {
  ...
  if (rank != NULL && is_init == 1 && is_final == 0) {
...
void *pool_alloc(size_t size, int isHost) {
  ...
  if (pool_size[*token & POOL_MASK]) {
    int i = 0;
    // roll backwards up to META_MPI_POOL_LOOKBACK cells to look for a big
    // enough buffer
    for (i = 0; i < META_MPI_POOL_LOOKBACK &&
                pool_size[(*token - i) & POOL_MASK] < size;
         i++)
      ;
    ...
    if (i < META_MPI_POOL_LOOKBACK) {
      ...
      // then notify (possibly other threads) that it's not available by zeroing
      // out the size
      pool_size[(*token - i) & POOL_MASK] = 0;

    } else { // Otherwise just pull what's at the token, realloc it and move on
      // When things are pushed back to the pool, they are added back with their
      // minimum size, so they might actually be bigger than they seem. In these
      // cases realloc should just resize the buffer in-place.
      ...
      if (retval == NULL)
        fprintf(
            stderr,
            "ERROR: in pool_alloc, realloc of pool buffer was unsuccessful!\n");
    }
...
// Only free moves the token, this is what causes the "ratchet ring queue"
// behavior
// If the same buffer is used META_MPI_POOL_SIZE times in a row, all other pool
// buffers will get freed by it constantly being dropped one cell forward
// NOT THREADSAFE
...
// If a non-NULL size pointer is provided, it returns the sizeof the
// meta_type_id there as well
 ...
 * Should only be used internally, and typically only once per application
 * launch
 */
...
void register_mpi_request(request_record_type type, request_record *request) {
  ...
  if (record_queue.type == uninit)
    init_record_queue();
  recordQueueNode *record =
      (recordQueueNode *)pool_alloc(sizeof(recordQueueNode), 0);
  record->type = type;
  record->next = NULL;
  ...
  // of the other three types (it will just copy unused garbage for the smaller
  // ones)
  ...
  // Enqueue the node
  // FIXME: Make hazard aware! (Or at least put in the stubs to finish later
  // once the HP
...
/**
 * This callback finishes setting up the MPI_ISend for an asynchronous
 * send_packed It should be internally registered to the device-to-host copy of
 * the relevant buffer
 * \param data The callback payload, containing a reference to this functino and
 * an sp_callback_payload with the information necessary to launch the MPI_ISend
 */
void sp_isend_cb(meta_callback *data) {
  ...
  MPI_Isend(call_pl->host_packed_buf, call_pl->buf_leng, mpi_type,
            call_pl->dst_rank, call_pl->tag, MPI_COMM_WORLD, call_pl->req);
  // Assemble a request
  ...
  // once the Isend is invoked and the request is submitted, we can remove the
  // payload
...
void meta_mpi_packed_face_send(int dst_rank, void *packed_buf, size_t buf_leng,
  ...
  if (run_mode == metaModePreferCUDA) {
    ...
    if (async) {
      MPI_Isend(packed_buf, buf_leng, mpi_type, dst_rank, tag, MPI_COMM_WORLD,
                req);
      // FIXME: Build a record around the request, to be finished by a helper
      // later
    } else {
    ...
    if (async) {
      ...
      meta_callback *call =
          (meta_callback *)pool_alloc(sizeof(meta_callback), 0);
      call->callback_mode = run_mode;
      call->callback_func = sp_isend_cb;
      // Assemble the callback's necessary data elements
      sp_callback_payload *call_pl =
          (sp_callback_payload *)pool_alloc(sizeof(sp_callback_payload), 0);
      ...
      // Copy the buffer to the host, with the callback chain needed to complete
      // the transfer
      error |= meta_copy_d2h(packed_buf_host, packed_buf, buf_leng * type_size,
                             0, call, NULL);
    } else {
...
/**
 * After an asynchronous recv_packed performs its host-to-device copy, the
 * host-staging buffer should be returned to the pool for reuse
 * \param data The callback payload which should be internally registered to the
 * host-to-device transfer, and contains a reference to this function and an
 * rp_callback_payload with the pointers to release back into the pool
 */
...
void rp_helper(request_record *rp_request) {
  // Paul, added this short circuit to prevent GPU-direct RPs from trying to do
  // a H2D copy and free a host buffer
  ...
  rp_callback_payload *call_pl =
      (rp_callback_payload *)pool_alloc(sizeof(rp_callback_payload), 0);
  ...
  // and invoke the async copy with the callback specified and the host pointer
  // as the payload
  meta_copy_h2d(rp_request->rp_rec.dev_packed_buf,
                rp_request->rp_rec.host_packed_buf, rp_request->rp_rec.buf_size,
                1, call, NULL);
}
...
void meta_mpi_packed_face_recv(int src_rank, void *packed_buf, size_t buf_leng,
  ...
  if (run_mode == metaModePreferCUDA) {
    ...
    if (async) {
      MPI_Irecv(packed_buf, buf_leng, mpi_type, src_rank, tag, MPI_COMM_WORLD,
                req);
      // TODO  ? Paul 2015.09.09
      // I see no reason to register the request, there is no need for a helper
      // to free the non-existent host buffer If we eventually allow the user to
      // "fast-forward" the registry to ensure a specific request completes,
      // then we will need to add both this and send_packed to the registry
      // Assemble a request_record
      // request_record * rec = (request_record *)
      // pool_alloc(sizeof(request_record), 0); rec->rp_rec.req = req;
      ...
      MPI_Recv(packed_buf, buf_leng, mpi_type, src_rank, tag, MPI_COMM_WORLD,
               &status);
    }
  } else {
#endif // WITH_MPI_GPU_DIRECT
    ...
    // FIXME: Add proper callback-related functionality to allow the pack and
    ...
    if (async) {
      ...
      // Assemble a request_record
      request_record *rec =
          (request_record *)pool_alloc(sizeof(request_record), 0);
      rec->rp_rec.req = req;
      ...
      error |= meta_copy_h2d(packed_buf, packed_buf_host, buf_leng * type_size,
                             0, NULL, NULL);
...
/**
 * To complete an asynchronous pack_and_send this callback is triggered after
 * the packing kernel and device-to-host tranfser, to finish setting up the
 * MPI_ISend
 * \param data The callback payload passed through from the device-to-host copy,
 * containing a reference to this function and a sap_callback_payload with the
 * information necessary to start the MPI_ISend
 */
void sap_isend_cb(meta_callback *data) {
  ...
  if (call_pl->host_packed_buf != NULL) { // No GPUDirect
    ...
    MPI_Isend(call_pl->dev_packed_buf, call_pl->buf_leng, mpi_type,
              call_pl->dst_rank, call_pl->tag, MPI_COMM_WORLD, call_pl->req);
  }
  // Assemble a request
  request_record *rec = (request_record *)pool_alloc(sizeof(request_record), 0);
...
void meta_mpi_pack_and_send_face(a_dim3 *grid_size, a_dim3 *block_size,
  ...
  // do a send, if asynchronous, this should be a callback for when the async
  // D2H copy finishes
#ifdef WITH_MPI_GPU_DIRECT
  // GPUDirect version
  if (run_mode == metaModePreferCUDA) {
    ...
    if (async) {
      ...
      // Assemble the callback's necessary data elements
      sap_callback_payload *call_pl =
          (sap_callback_payload *)pool_alloc(sizeof(sap_callback_payload), 0);
      ...
      // call_pl->host_packed_buf, call_pl->dev_packed_buf, call_pl->buf_leng,
      // call_pl->tag, call_pl->req);
      ...
      meta_copy_d2h_cb(packed_buf_host, packed_buf, size * type_size, 1, call,
                       call_pl);
      // This was no longer valid after switching to host staging, as required
      // by GPUDirect/CUDACallback interaction meta_pack_face_cb(grid_size,
      // block_size, packed_buf, buf, face, type, 1, call, call_pl);
    } else {
      error |=
          meta_pack_face(grid_size, block_size, packed_buf, buf, face, type, 0);
    ...
    if (async) {
      ...
      call_pl->req = req;
      call->data_payload = call_pl;
      meta_copy_d2h(packed_buf_host, packed_buf, size * type_size, 1, call,
                    NULL);
    } else {
      error |= meta_pack_face(grid_size, block_size, packed_buf, buf, face,
                              type, 0, NULL, NULL, NULL, NULL, NULL);
      error |= meta_copy_d2h(packed_buf_host, packed_buf, size * type_size, 0,
                             NULL, NULL);
...
/**
 * After a recv_and_unpack operation, this callback fires after the unpacking
 * kernel to release the host-staging buffer back to the pool
 * \param data The callback payload returned from the unpacking kernel, which
 * contains a reference to this function and a rap_callback_payload containing
 * the pointers that should be released back to the pool
 */
void rap_freebufs_cb(meta_callback *data) {
  rap_callback_payload *call_pl = (rap_callback_payload *)(data->data_payload);
  if (call_pl->host_packed_buf != NULL)
    pool_free(call_pl->host_packed_buf, call_pl->buf_size, 1);
...
void rap_helper(request_record *rap_request) {
  ...
  // and invoke the async copy with the callback specified and the host pointer
  // as the payload
  rap_callback_payload *call_pl =
      (rap_callback_payload *)pool_alloc(sizeof(rap_callback_payload), 0);
  ...
  meta_unpack_face((rap_request->rap_rec.grid_size[0] == 0
                        ? NULL
                        : &(rap_request->rap_rec.grid_size)),
                   (rap_request->rap_rec.block_size[0] == 0
                        ? NULL
                        : &(rap_request->rap_rec.block_size)),
                   rap_request->rap_rec.dev_packed_buf,
                   rap_request->rap_rec.dev_buf, rap_request->rap_rec.face,
                   rap_request->rap_rec.type, 1, call, NULL, NULL, NULL, NULL);
}
...
void meta_mpi_recv_and_unpack_face(a_dim3 *grid_size, a_dim3 *block_size,
  ...
  if (run_mode == metaModePreferCUDA) {
    if (async) {
      ...
      MPI_Recv(packed_buf, size, mpi_type, src_rank, tag, MPI_COMM_WORLD,
               &status);
    }
  } else {
#endif // WITH_MPI_GPU_DIRECT
    ...
    packed_buf_host = pool_alloc(size * type_size, 1);
    // do a receive, if asynchronous, we need to implement a callback to launch
    // the H2D copy
    // FIXME: Add proper callback-related functionality to allow the pack and
    // copy to be asynchronous
    if (async) {
      MPI_Irecv(packed_buf_host, size, mpi_type, src_rank, tag, MPI_COMM_WORLD,
                req);
      // TODO: Register the transfer, with host pointer set to packed_buf_host
      // Helper will recognize the non-null pointer and do an h2d transfer and
      // unpack
      ...
      error |= meta_unpack_face(grid_size, block_size, packed_buf, buf, face,
                                type, 0, NULL, NULL, NULL, NULL, NULL);
