...
/** Maintain a static set of queues for all builtin operations (transfers and
 * kernels) */
...
/**
 * Struct to hold OpenCL wrapper function pointers that are only relevant to
 * profiling
 */
...
/**
 * Struct to hold OpenMP wrapper function pointers that are only relevant to
 * profiling
 */
...
a_err cl_get_event_node(metaTimerQueue *queue, char *ename,
  ...
  while (1) {
    ...
    while (flag > 0) {
      ...
      printf("passing profiling event '%s' (timer side) %lu and the error is
      ...
      */
      ...
      if (strcmp(temp->name, ename) == 0) // found the event
      {
        ...
        size_t return_bytes;
        clGetEventProfilingInfo(temp->event.opencl,CL_PROFILING_COMMAND_QUEUED,sizeof(cl_ulong),&start_time,&return_bytes);
        printf("POI profiling event '%s' (timer side)
...
/**
 * Internal: Take the copy of the front frame on the queue, and then remove it
 * from the queue Do nothing else with the frame's copy, the caller should
...
__attribute__((constructor(104))) void metaTimersInit() {
  ...
  // Create a timer with a special sentinel type that we can copy into each of
  // the queue sentinels
  meta_timer *sentinel;
  ...
  metaBuiltinQueues[k_csr].head = metaBuiltinQueues[k_csr].tail =
      (metaTimerQueueNode *)malloc(sizeof(metaTimerQueueNode));
  ...
  // Init the csr kernel queue
  metaBuiltinQueues[k_crc].head = metaBuiltinQueues[k_crc].tail =
      (metaTimerQueueNode *)malloc(sizeof(metaTimerQueueNode));
 ...
 * \param level The verbosity level to report (0 = silent, 1 = aggregate, 2 =
 * individual cals, 3 = unimplemented)
 * \todo figure out how to handle encountered events which have not completed
 * (do we put them back on the queue? register a callback? force the
 * command_queue to finish?
  ...
  while ((val = metaTimerDequeue(timer, queue)) != -1) {
    ...
    if (level == 0) {
      if (timer->mode == metaModePreferOpenCL) {
        ...
        if (opencl_timing_funcs.metaOpenCLEventStartTime != NULL)
          ret = (*(opencl_timing_funcs.metaOpenCLEventStartTime))(timer->event,
                                                                  &start);
        if (opencl_timing_funcs.metaOpenCLEventEndTime != NULL)
          ret = (*(opencl_timing_funcs.metaOpenCLEventEndTime))(timer->event,
                                                                &end);
    ...
    if (level >= 1) {
      if (timer->mode == metaModePreferGeneric) {
        ...
        // TODO add a check to cudaEventQuery to make sure frame->event.cuda[1]
        // is finished (PS: This should be done in the CUDA backend and an
        // appropriate backend-agnostic error returned if it's not ready)
        if (cuda_timing_funcs.metaCUDAEventElapsedTime != NULL)
          ret = (*(cuda_timing_funcs.metaCUDAEventElapsedTime))(&temp_t,
                                                                timer->event);
      } else if (timer->mode == metaModePreferOpenCL) {
        // TODO add a check via clGetEventInfo to make sure the event has
        // completed
        if (opencl_timing_funcs.metaOpenCLEventStartTime != NULL)
          ret = (*(opencl_timing_funcs.metaOpenCLEventStartTime))(timer->event,
                                                                  &start);
        ...
        if (openmp_timing_funcs.metaOpenMPEventElapsedTime != NULL)
          ret = (*(openmp_timing_funcs.metaOpenMPEventElapsedTime))(
              &temp_t, timer->event);
      }
    ...
    if (level >= 2) {
      ...
      fprintf(stderr,
              "%s [%lu] on [%lu]bytes took [%f]ms, with [%f]MB/s approximate "
              "bandwidth.\n",
              queue->name, count, timer->size, temp_t,
              (timer->size > 0 && temp_t > 0) ? timer->size * .001 / temp_t
                                              : 0.0);
    }
