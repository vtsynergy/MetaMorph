...
void PrototypeHandler::run(const MatchFinder::MatchResult &Result) {
  ...
  if (func) {
    ...
    doxygenArgs += "\\param queue the cl_command_queue the kernel is being prepared to run on (to lookup implementation)\n";
    int pos = 0;
...
//    doxygen += "\\param type the MetaMorph type of the function\n";
    doxygenEnqueue += "\\param queue the cl_command_queue on which to enqueue the kernel\n";
    //Add typical sizing parameters to the launch parameter list
    launchParams += ", size_t (*grid_size)[3], size_t (*block_size)[3], size_t (*meta_offset)[3], int async, cl_event * event";
    ...
    sizeCheck += "  a_bool nullBlock = (block_size != NULL && (*block_size)[0] == 0 && (*block_size)[1] == 0 && (*block_size)[2] == 0);\n";
    sizeCheck += "  size_t grid[3];\n";
    if (work_group_size[3] == 0 && !singleWorkItem) {
      sizeCheck += "  size_t block[3] = METAMORPH_OCL_DEFAULT_BLOCK_3D;\n";
    } else if (singleWorkItem) {
      sizeCheck += "  size_t block[3] = {1, 1, 1};\n";
    } else {
      sizeCheck += "  size_t block[3] = {" + std::to_string(work_group_size[0]) + ", " + std::to_string(work_group_size[1]) + ", " + std::to_string(work_group_size[2]) + "};\n";
    }
    sizeCheck += "  int iters;\n\n";
    sizeCheck += "  //Default runs a single workgroup\n";
    sizeCheck += "  if (grid_size == NULL || block_size == NULL) {\n";
    sizeCheck += "    grid[0] = block[0];\n    grid[1] = block[1];\n    grid[2] = block[2];\n";
    sizeCheck += "    iters = 1;\n";
    sizeCheck += "  } else {\n";
    if (work_group_size[3] == 1) {
      sizeCheck += "    if (!(block[0] == (*block_size)[0] && block[1] == (*block_size)[1] && block[2] == (*block_size)[2])) {\n";
      ...
      sizeCheck += "    if (!(block[0] == (*block_size)[0] && block[1] == (*block_size)[1] && block[2] == (*block_size)[2])";
      if (singleWorkItem) sizeCheck += " && !(grid[0] == 1 && grid[1] == 1 && grid[2] == 1)";
      sizeCheck += ") {\n";
    ...
    sizeCheck += "    grid[0] = (*grid_size)[0] * (nullBlock ? 1 : (*block_size)[0]);\n";
    sizeCheck += "    grid[1] = (*grid_size)[1] * (nullBlock ? 1 : (*block_size)[1]);\n";
    sizeCheck += "    grid[2] = (*grid_size)[2] * (nullBlock ? 1 : (*block_size)[2]);\n";
    sizeCheck += "    block[0] = (*block_size)[0];\n";
    sizeCheck += "    block[1] = (*block_size)[1];\n";
    sizeCheck += "    block[2] = (*block_size)[2];\n";
    sizeCheck += "    iters = (*grid_size)[2];\n";
    sizeCheck += "  }\n";
