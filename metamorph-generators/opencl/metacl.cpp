...
void PrototypeHandler::run(const MatchFinder::MatchResult &Result) {
  ...
  if (func) {
    ...
    doxygenGeneral += "\\param queue the cl_command_queue the kernel is being prepared to run on (to lookup and/or enqueue associated cl_kernel)\n";
    ...
    regCheck += "  if (frame->" + filename + "_init != 1) return CL_INVALID_PROGRAM;\n";
    ...
    for (ParmVarDecl * parm : func->parameters()) {
      ...
      if (info->isGlobalOrConst) {
        ...
        doxygenArgs += "\\param " + info->name + " a cl_mem buffer, must internally store " + info->hostType + " types\n";
        ...
        setArgs += ERROR_CHECK("  ", "retCode", "OpenCL kernel argument assignment error (arg: \\\"" + info->name + "\\\", host wrapper: \\\"" + host_func + "\\\")");
        doxygenArgs += "\\param " + info->name + "_num_local_elems allocate __local memory space for this many " + info->hostType + " elements\n";
      } else {
        ...
        setArgs += ERROR_CHECK("  ", "retCode", "OpenCL kernel argument assignment error (arg: \\\"" + info->name + "\\\", host wrapper: \\\"" + host_func + "\\\")");
        doxygenArgs += "\\param " + info->name + " scalar parameter of type \"" + info->hostType + "\"\n";
      }
      pos++;
    }
...
//    doxygen += "\\param type the MetaMorph type of the function\n";
    ...
    if (GridBlockSizes.getValue()) {
      ...
      doxygenEnqueue += "\\param grid_size a size_t[3] providing the number of workgroups in the X, Y, Z dimensions\n";
      doxygenEnqueue +="\\param block_size a size_t[3] providing the workgroup size in the X, Y, Z dimensions";
    } else {
      outerSizeName = "global_size", innerSizeName = "local_size";
      doxygenEnqueue +="\\param global_size a size_t[3] providing the global number of workitems in the X, Y, Z dimensions";
      doxygenEnqueue +="\\param local_size a size_t[3] providing the workgroup size in the X, Y, Z dimensions";
    }
    if (singleWorkItem) {
      doxygenEnqueue += " (detected as single-work-item, must be {1, 1, 1})\n";
    } else if (work_group_size[3] == 1) {
      doxygenEnqueue += " (work_group_size_hint attribute suggests {" + std::to_string(work_group_size[0]) + ", " + std::to_string(work_group_size[1]) + ", " + std::to_string(work_group_size[2]) + "})\n";
    } else if (work_group_size[3] == 2) {
      doxygenEnqueue += " (reqd_work_group_size attribute requires {" + std::to_string(work_group_size[0]) + ", " + std::to_string(work_group_size[1]) + ", " + std::to_string(work_group_size[2]) + "})\n";
    } else {
      doxygenEnqueue += "\n";
    }
    launchParams += ", size_t (*" + outerSizeName + ")[3], size_t (*" + innerSizeName + ")[3], size_t (*meta_offset)[3], int async, cl_event * event";
    ...
    sizeCheck += "  a_bool nullBlock = (" + innerSizeName + " != NULL && (*" + innerSizeName + ")[0] == 0 && (*" + innerSizeName + ")[1] == 0 && (*" + innerSizeName + ")[2] == 0);\n";
    sizeCheck += "  size_t _global_size[3];\n";
    if (work_group_size[3] == 0 && !singleWorkItem) {
      sizeCheck += "  size_t _local_size[3] = METAMORPH_OCL_DEFAULT_BLOCK_3D;\n";
    } else if (singleWorkItem) {
      sizeCheck += "  size_t _local_size[3] = {1, 1, 1};\n";
    } else {
      sizeCheck += "  size_t _local_size[3] = {" + std::to_string(work_group_size[0]) + ", " + std::to_string(work_group_size[1]) + ", " + std::to_string(work_group_size[2]) + "};\n";
    }
    sizeCheck += "  size_t _temp_offset[3];\n";
    sizeCheck += "  if (meta_offset == NULL) { _temp_offset[0] = 0, _temp_offset[1] = 0, _temp_offset[2] = 0;}\n";
    sizeCheck += "  else { _temp_offset[0] = (*meta_offset)[0], _temp_offset[1] = (*meta_offset)[1], _temp_offset[2] = (*meta_offset)[2]; }\n";
    sizeCheck += "  const size_t _meta_offset[3] = {_temp_offset[0], _temp_offset[1], _temp_offset[2]};\n";
    sizeCheck += "  int iters;\n\n";
    sizeCheck += "  //Default runs a single workgroup\n";
    sizeCheck += "  if (" + outerSizeName + " == NULL || " + innerSizeName + " == NULL) {\n";
    sizeCheck += "    _global_size[0] = _local_size[0];\n    _global_size[1] = _local_size[1];\n    _global_size[2] = _local_size[2];\n";
    sizeCheck += "    iters = 1;\n";
    sizeCheck += "  } else {\n";
    if (work_group_size[3] == 1) {
      sizeCheck += "    if (!(_local_size[0] == (*" + innerSizeName + ")[0] && _local_size[1] == (*" + innerSizeName + ")[1] && _local_size[2] == (*" + innerSizeName + ")[2])) {\n";
      sizeCheck += "      fprintf(stderr, \"Warning: kernel " + func->getNameAsString() + " suggests a workgroup size of {" + std::to_string(work_group_size[0]) + ", " + std::to_string(work_group_size[1]) + ", " + std::to_string(work_group_size[2]) + "} at \%s:\%d\\n\", __FILE__, __LINE__);\n";
      sizeCheck += "    }\n";

    } else if (work_group_size[3] == 2 || singleWorkItem) {
      sizeCheck += "    if (!(_local_size[0] == (*" + innerSizeName + ")[0] && _local_size[1] == (*" + innerSizeName + ")[1] && _local_size[2] == (*" + innerSizeName + ")[2])";
      if (singleWorkItem) sizeCheck += " && !(_global_size[0] == 1 && _global_size[1] == 1 && _global_size[2] == 1)";
      sizeCheck += ") {\n";
      sizeCheck += "      fprintf(stderr, \"Error: kernel " + func->getNameAsString() + " requires a workgroup size of {" + std::to_string(work_group_size[0]) + ", " + std::to_string(work_group_size[1]) + ", " + std::to_string(work_group_size[2]) + "}, aborting launch at \%s:\%d\\n\", __FILE__, __LINE__);\n";
      sizeCheck += "      return CL_INVALID_WORK_GROUP_SIZE;\n";
      sizeCheck += "    }\n";
    }
    if (GridBlockSizes.getValue()) {
    sizeCheck += "    _global_size[0] = (*" + outerSizeName + ")[0] * (nullBlock ? 1 : (*" + innerSizeName + ")[0]);\n";
    sizeCheck += "    _global_size[1] = (*" + outerSizeName + ")[1] * (nullBlock ? 1 : (*" + innerSizeName + ")[1]);\n";
    sizeCheck += "    _global_size[2] = (*" + outerSizeName + ")[2] * (nullBlock ? 1 : (*" + innerSizeName + ")[2]);\n";
    } else {
    //OpenCL will return CL_INVALID_WORK_GROUP_SIZE if it's not an even divisor of global size, don't need to check
    ...
    int workDim = 3;
    std::string globalSize = "_global_size", localSize = "(nullBlock ? NULL : _local_size)";
    ...
    if (singleWorkItem || (work_group_size[3] != 0 && work_group_size[0] == 1 && work_group_size[1] == 1 && work_group_size[2] == 1)) {
      ...
      enqueue += "  retCode = clEnqueueNDRangeKernel(frame->queue, " + framed_kernel + ", " + std::to_string(workDim) + ", &_meta_offset[0], " + globalSize + ", " + localSize + ", " + eventWaitListSize + ", " + eventWaitList + ", " + retEvent + ");\n";
      enqueue += ERROR_CHECK("  ", "retCode", "OpenCL kernel enqueue error (host wrapper: \\\"" + host_func + "\\\")");
    }
    enqueue += "  if (!async) {\n";
    enqueue += "    retCode = clFinish(frame->queue);\n";
    enqueue += ERROR_CHECK("    ", "retCode", "OpenCL kernel execution error (host wrapper: \\\"" + host_func + "\\\")");
    enqueue += "  }\n";
...
//Moved      cache->runOnceInit += "  }\n";
      ...
      cache->runOnceDeinit += "      free((char *)__metacl_" + outFile + "_current_frame->" + file + "_progSrc);\n";
