...
static void omp_dotProd_kernel_ul( unsigned long * __restrict__ data1, unsigned long * __restrict__ data2, size_t (* array_size)[3], size_t (* arr_start)[3], size_t (* arr_end)[3], unsigned long * reduction_var)
{
	unsigned long sum = 0;
	int ni, nj, nk;
...
static void omp_dotProd_kernel_in( int * __restrict__ data1, int * __restrict__ data2, size_t (* array_size)[3], size_t (* arr_start)[3], size_t (* arr_end)[3], int * reduction_var)
{
	int sum = 0;
	int ni, nj, nk;
...
static void omp_dotProd_kernel_ui( unsigned int * __restrict__ data1, unsigned int * __restrict__ data2, size_t (* array_size)[3], size_t (* arr_start)[3], size_t (* arr_end)[3], unsigned int * reduction_var)
{
	unsigned int sum = 0;
	int ni, nj, nk;
...
//reduce
static void omp_reduce_kernel_db( double * __restrict__ data, size_t (* array_size)[3], size_t (* arr_start)[3],  size_t (* arr_end)[3], double * reduction_var)
{
	int ni, nj, nk;
	double sum = 0;
...
static void omp_reduce_kernel_fl( float *__restrict__  data, size_t (* array_size)[3], size_t (* arr_start)[3], size_t (* arr_end)[3], float * reduction_var)
{
	int ni, nj, nk;
	float sum = 0;
...
static void omp_reduce_kernel_ul( unsigned long * __restrict__ data, size_t (* array_size)[3], size_t (* arr_start)[3], size_t (* arr_end)[3], unsigned long * reduction_var)
{
	int ni, nj, nk;
	unsigned long sum = 0;
...
static void omp_reduce_kernel_in( int * __restrict__ data, size_t (* array_size)[3], size_t (* arr_start)[3], size_t (* arr_end)[3], int * reduction_var)
{
	int ni, nj, nk;
	int sum = 0;
...
static void omp_reduce_kernel_ui( unsigned int * __restrict__ data, size_t (* array_size)[3], size_t (* arr_start)[3], size_t (* arr_end)[3], unsigned int * reduction_var)
{
	int ni, nj, nk;
	unsigned int sum = 0;
...
static void omp_transpose_2d_face_kernel_db(double * __restrict__ indata, double * __restrict__ outdata, size_t (* arr_dim_xy)[3], size_t (* tran_dim_xy)[3])
{
	int ni, nj;
	unsigned int sum = 0;
...
static void omp_transpose_2d_face_kernel_fl(float * __restrict__ indata, float * __restrict__ outdata, size_t (* arr_dim_xy)[3], size_t (* tran_dim_xy)[3])
{
	int ni, nj;
	unsigned int sum = 0;
...
static void omp_transpose_2d_face_kernel_ul(unsigned long * __restrict__ indata, unsigned long * __restrict__ outdata, size_t (* arr_dim_xy)[3], size_t (* tran_dim_xy)[3])
{
...
static void omp_transpose_2d_face_kernel_in(int * __restrict__ indata, int * __restrict__ outdata, size_t (* arr_dim_xy)[3], size_t (* tran_dim_xy)[3])
{
	int ni, nj;
	unsigned int sum = 0;
...
static void omp_transpose_2d_face_kernel_ui(unsigned int * __restrict__ indata, unsigned int * __restrict__ outdata, size_t (* arr_dim_xy)[3], size_t (* tran_dim_xy)[3])
{
	int ni, nj;
	unsigned int sum = 0;
...
static int get_pack_index (int idx, meta_2d_face_indexed *face, int * __restrict__ remain_dim) {
        int pos;
        int i, j, k, l;
        int a[METAMORPH_FACE_MAX_DEPTH];
...
int omp_dotProd(size_t (* grid_size)[3], size_t (* block_size)[3], void * data1, void * data2, size_t (* array_size)[3], size_t (* arr_start)[3],  size_t (* arr_end)[3], void * reduction_var, meta_type_id type, int async)
{
		...
		switch (type) {
			case a_db:
				omp_dotProd_kernel_db((double*) data1, (double*) data2, array_size, arr_start, arr_end, (double*) reduction_var);
			break;

			case a_fl:
				omp_dotProd_kernel_fl((float*) data1, (float*) data2, array_size, arr_start, arr_end, (float*) reduction_var);
			break;

			case a_ul:
				omp_dotProd_kernel_ul((unsigned long*) data1, (unsigned long*) data2, array_size, arr_start, arr_end, (unsigned long*) reduction_var);
			break;

			case a_in:
				omp_dotProd_kernel_in((int*) data1, (int*) data2, array_size, arr_start, arr_end, (int*) reduction_var);
			break;

			case a_ui:
				omp_dotProd_kernel_ui((unsigned int*) data1, (unsigned int*) data2, array_size, arr_start, arr_end, (unsigned int*) reduction_var);
			break;

			default:
				fprintf(stderr, "Error: Function 'omp_dotProd' not implemented for selected type!\n");
			break;
		}
...
int omp_reduce(size_t (* grid_size)[3], size_t (* block_size)[3], void * data, size_t (* array_size)[3], size_t (* arr_start)[3],  size_t (* arr_end)[3], void * reduction_var, meta_type_id type, int async)
{
	...
	switch (type) {
		case a_db:
			omp_reduce_kernel_db((double*) data, array_size, arr_start, arr_end, (double*) reduction_var);
		break;

		case a_fl:
			omp_reduce_kernel_fl((float*) data, array_size, arr_start, arr_end, (float*) reduction_var);
		break;

		case a_ul:
			omp_reduce_kernel_ul((unsigned long*) data, array_size, arr_start, arr_end, (unsigned long*) reduction_var);
		break;

		case a_in:
			omp_reduce_kernel_in((int*) data, array_size, arr_start, arr_end, (int*) reduction_var);
		break;

		case a_ui:
			omp_reduce_kernel_ui((unsigned int*) data, array_size, arr_start, arr_end, (unsigned int*) reduction_var);
		break;

		default:
			fprintf(stderr, "Error: Function 'omp_reduce' not implemented for selected type!\n");
		break;
	}
...
int omp_transpose_2d_face(size_t (* grid_size)[3], size_t (* block_size)[3], void * indata, void *outdata, size_t (* arr_dim_xy)[3], size_t (* tran_dim_xy)[3], meta_type_id type, int async)
{
	...
	switch (type) {
		case a_db:
			omp_transpose_2d_face_kernel_db((double*) indata,(double*) outdata, arr_dim_xy, tran_dim_xy);
		break;

		case a_fl:
			omp_transpose_2d_face_kernel_fl((float*) indata,(float*) outdata, arr_dim_xy, tran_dim_xy);
		break;

		case a_ul:
			omp_transpose_2d_face_kernel_ul((unsigned long*) indata,(unsigned long*) outdata, arr_dim_xy, tran_dim_xy);
		break;

		case a_in:
			omp_transpose_2d_face_kernel_in((int*) indata,(int*) outdata, arr_dim_xy, tran_dim_xy);
		break;

		case a_ui:
			omp_transpose_2d_face_kernel_ui((unsigned int*) indata,(unsigned int*) outdata, arr_dim_xy, tran_dim_xy);
		break;

		default:
			fprintf(stderr, "Error: Function 'omp_transpose_2d_face' not implemented for selected type!\n");
		break;
	}
...
int omp_pack_2d_face(size_t (* grid_size)[3], size_t (* block_size)[3], void *packed_buf, void *buf, meta_2d_face_indexed *face, int *remain_dim, meta_type_id type, int async)
{
	...
	switch (type) {
		case a_db:
			omp_pack_2d_face_kernel_db((double*) packed_buf, (double*) buf, face, remain_dim);
		break;

		case a_fl:
			omp_pack_2d_face_kernel_fl((float*) packed_buf, (float*) buf, face, remain_dim);
		break;

		case a_ul:
			omp_pack_2d_face_kernel_ul((unsigned long*) packed_buf, (unsigned long*) buf, face, remain_dim);
		break;

		case a_in:
			omp_pack_2d_face_kernel_in((int*) packed_buf, (int*) buf, face, remain_dim);
		break;

		case a_ui:
			omp_pack_2d_face_kernel_ui((unsigned int*) packed_buf, (unsigned int*) buf, face, remain_dim);
		break;
...
int omp_unpack_2d_face(size_t (* grid_size)[3], size_t (* block_size)[3], void *packed_buf, void *buf, meta_2d_face_indexed *face, int *remain_dim, meta_type_id type, int async)
{
	...
	switch (type) {
		case a_db:
			omp_unpack_2d_face_kernel_db((double*) packed_buf, (double*) buf, face, remain_dim);
		break;

		case a_fl:
			omp_unpack_2d_face_kernel_fl((float*) packed_buf, (float*) buf, face, remain_dim);
		break;

		case a_ul:
			omp_unpack_2d_face_kernel_ul((unsigned long*) packed_buf, (unsigned long*) buf, face, remain_dim);
		break;

		case a_in:
			omp_unpack_2d_face_kernel_in((int*) packed_buf, (int*) buf, face, remain_dim);
		break;

		case a_ui:
			omp_unpack_2d_face_kernel_ui((unsigned int*) packed_buf, (unsigned int*) buf, face, remain_dim);
		break;
